...
$('#txt').click(function () {
    ...
    fileTranscription = writeTranscriptFile(transcribed);
...
function uploadObject(fileTranscription) {

  console.log('we are uploading');

  s3.putObject({
    Bucket: "amazon-mic-files",
    Key: "test_file_transcribed",
    Body: fileTranscription
  },
  function(err,data){
    if (err) {
...
//writes transcription to text file
function writeTranscriptFile(transcribed) {
  fs.writeFile('transcription.txt', transcribed, (err) => {
    if (err) throw err;
    console.log('we wrote a text file');
  });
};
