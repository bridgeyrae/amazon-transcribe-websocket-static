...
function uploadObject(transcribed) {
  ...
  s3.putObject({
    ...
    Key: "test_file_transcribed", //update key with file name that is distinctive
    Body: transcribed
  },
  function(err,data){
    if (err) {
...
function getDate() {
  //get day in month
  var day = Date.prototype.getDate();
  //year
  ...
  //minutes
  var mins = Date.prototype.getMinutes();
  //seconds
  var secs = Date.protope.getSeconds();

  date = month+"/"+day+"/"+year+" "+hours+":"+mins+"."+secs
